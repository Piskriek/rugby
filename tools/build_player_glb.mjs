...
let manPath = path.join(manDir, 'Man Animated - Oct 2017', 'FBX', 'Animated Human.fbx');
if (!fs.existsSync(manPath)) {
  manPath = findFile(manDir, /^Animated Human\.fbx$/i) || findFile(manDir, /\.fbx$/i);
  if (!manPath) throw new Error('Animated Human.fbx not in zip');
  console.log('  using', manPath);
}
