...
check: (p) => (num(p, 'height') > 2 && num(p, 'tilt') > 0 && num(p, 'tilt') < 1.4 && num(p, 'fov') > 0.04 && num(p, 'fov') <= 1.2) ? ok() : bad(`bad rig: h=${num(p, 'height')} tilt=${num(p, 'tilt')} fov=${num(p, 'fov')}`) },
  ...
  /* ---------- DEFENSIVE LINE ---------- */
  /* SPEC_10 B2b / BATCH 03: `maxGapMetres` is emitted by trace.ts from the
   * live positional population: available defenders, the measured outermost
   * flank men, and zero or one sweeper selected by the active system's
   * sweeperDepth. No shirt-number role is assumed. The first ~1.2 s after a
   * phase change remains the legal reset transition, and the designed spacing
   * is up to 4.0 m; judge a re-formed line against the played system's spacing
   * + 0.6 m, with the existing 4.6 m floor. */
