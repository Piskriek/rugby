...
interface KindSpec {
  ...
  /** Restitution when it hits the deck. Defaults to 0.22 — deliberately NOT the
   *  default for soil, which lands and stays. */
  bounce?: number;
}

const SPEC: Record<ParticleKind, KindSpec> = {
  TURF:     { gravity: -22, drag: 2.6, wind: 0.25, life: [0.45, 0.95], size: [0.10, 0.22], additive: false, sway: 0, ground: true, rise: 0, soft: 0.9, bounce: 0.08 },
  /* MUD was the comedy. Life [0.6,1.25] s, size up to 0.30 m, and the default
   * 0.22 restitution meant a tackle threw thirty brown discs that bounced,
   * rolled and then SAT on the grass around the ruck for a second and a half —
   * which at 1998 resolution reads exactly like what the user called blood
   * puddles. Turf that comes up at a breakdown is thrown and gone: short life,
   * smaller clods, an upward bias, and no bounce at all so nothing pools. */
  MUD:      { gravity: -26, drag: 3.1, wind: 0.12, life: [0.24, 0.55], size: [0.07, 0.17], additive: false, sway: 0, ground: true, rise: 0.75, soft: 1.0, bounce: 0 },
...
export class ThreeParticles {
  ...
  emit(kind: ParticleKind, o: EmitOpts) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.bounce[idx] = S.ground ? (o.bounce ?? S.bounce ?? 0.22) : 0;
