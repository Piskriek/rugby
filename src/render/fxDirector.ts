  ...
  update(d: Director, cond: Conditions, dt: number): FxPulse {
    ...
    for (let i = 0; i < live.length; i++) {
      ...
      if (down && !wasDown) {
        ...
        this.fx?.emit(cond.mud > 0.5 ? 'MUD' : 'TURF', {
          ...
          bounce: firm * 0.34, scale: 0.9 + p.size * 0.25,
        });
        if (dust > 0.16) {
          this.fx?.emit('DUST', {
    ...
    if (d.bd) {
      ...
      if (d.bd.stage === 'RUCK' && !d.bd.ruckFormed && this.lastRuck !== stamp) {
        ...
        this.fx?.emit(cond.mud > 0.5 ? 'MUD' : 'TURF', {
          count: 12, x, y: 0.05 * S, z, speed: 5, up: 3, spread: 1.8,
          colorA: ca, colorB: cb, bounce: firm * 0.28,
        });
        this.env?.addScar(d.bd.contactX, d.bd.contactZ, 0.9);
      }
    }
