...
export const STAND: Pose = {
  ...
  /* PLAYTEST 4 — arms HANG: the old a0.12/e0.55 held the forearms up in
   * front like a waiter's basket. Near-zero shoulder pitch and a slack
   * elbow reads as a man standing, not a man holding something. */
  aL: 0.03, aR: 0.03, abL: 0.08, abR: 0.08, eL: 0.16, eR: 0.16,
...
function genRun(dur: number, amp: number, lean: number, dip: number, armBend: number): Clip {
  const c = (s: number): Partial<Pose> => ({
    ...
    headP: -0.07 + lean * 0.35, abL: 0.08, abR: 0.08,
  });
...
export const CLIPS: Record<string, Clip> = {
  /* PLAYTEST 4 — armBend walked back: the old walk (0.7) strolled with a
   * permanent 40-degree basket; jog/run/sprint bent past 90. A walk swings
   * near-straight arms; a runner's elbow sits just under 90. */
  jog: genRun(0.72, 0.5, 0.12, 0.045, 1.0),
  run: genRun(0.58, 0.85, 0.24, 0.08, 1.15),
  sprint: genRun(0.46, 1.15, 0.36, 0.12, 1.3),
  walk: genRun(1.05, 0.32, 0.05, 0.02, 0.25),

  idle: {
    dur: 2.6, loop: true, keys: [
      /* PLAYTEST 4: arms swing LOOSELY when unused — a slow counter-phase
       * pendulum on the shoulders with the elbows breathing, on top of the
       * hanging stance. */
      { t: 0, e: 's', p: { hip: 0.945, aL: -0.05, aR: 0.08, eL: 0.18, eR: 0.26 } },
      { t: 0.5, e: 's', p: { hip: 0.925, aL: 0.09, aR: -0.03, eL: 0.28, eR: 0.16, headY: 0.08, roll: 0.02, lean: 0.07 } },
    ],
  },
