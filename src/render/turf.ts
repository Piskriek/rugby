...
function paintRoughness(
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      px[i] = 255;                        // AO channel: no ambient-occlusion pass
                                            // ships in this pipeline, so the map
                                            // stays open and the grounding cue is
                                            // the real shadow map plus the contact
                                            // blob the squad manager draws
