/**
 * ragdollKernel.ts — the 11-node Verlet body the BAKED falls were simulated with.
 *
 * THE NAME. This file and `ragdoll.ts` are two solvers, deliberately apart, and
 * they used to be the same path. `ragdoll.ts` is the one the game runs live:
 * 20 particles that are the rig's own bones, joint limits, pair contact, soft
 * pins for the wrap, and a ground it can kick dust off. This one is the cheap
 * 11-node kernel whose output was baked, offline, into `ragdollClips.json`, and
 * it still has two jobs after the live solver shipped: it is the bake source, and
 * it is the FALL THE LEGACY TIER PLAYS BACK, where a per-frame solve is exactly
 * what that tier exists to avoid. Baking needs a body with no view in it, which
 * is what this is; the two must not be merged, because a solver that drives bones
 * cannot be tested the way this one can.
 *
 * A Verlet / position-based ragdoll solver.
 *
